...
import {
	type BulkAdjustmentsResponse,
	importStockAdjustmentsCsv,
} from "@/api/stocks";

/** Batas ukuran file sesuai contract.md bagian 20 (POST /stocks/bulk-adjustments). */
const MAX_FILE_SIZE = 10 * 1024 * 1024; // 10 MB

interface ImportStockCsvModalProps {
	opened: boolean;
	onClose: () => void;
}

export function ImportStockCsvModal({
	opened,
	onClose,
}: ImportStockCsvModalProps) {
	...
	const [result, setResult] = useState<BulkAdjustmentsResponse | null>(null);

	const mutation = useMutation({
		mutationFn: importStockAdjustmentsCsv,
		onSuccess: (data) => {
			setResult(data);
	...
	return (
		...
		>
			<Stack gap="md">
				<FileInput
					label="File CSV"
					description="Header: sku,adjustment_type,quantity,reason. Maksimum 10 MB."
